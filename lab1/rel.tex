...
\begin{document}
	...
	\subsection{a)}
		...
		\par
		Considerando que a água é um meio linear, e portanto $\vec{J} = \sigma \vec{E}$, que $\vec{E}$ e $\vec{J}$ são uniformes dentro do tanque e que $\vec{J} // \vec{n}$, então:
		
		$$ U = RI \eq R = \dfrac{U}{I} = \dfrac{\int\limits_{\text{placa}(0)}^{\text{placa}(4)} \vec{E} \cdot d\vec{s}}{\int\limits_{S_{(4)}}(\vec{J} \cdot \vec{n})dS} = \dfrac{aE}{\sigma Ewl} = \dfrac{a}{\sigma wl}$$
		\par
	...
	Recorrendo ao método das imagens, usando a informação presente na figura 1, e usando a expressão obtida no laboratório inicial para o cálculo do potencial entre condutores filiformes, tem-se:
